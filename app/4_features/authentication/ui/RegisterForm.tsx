...
export function RegisterForm(): JSX.Element {
    ...
    const submitHandler = async (formData: FormData) => {
        ...
        if(result.customer) {
            navigation.push(`/${local}/authorization?registration=success`)
        }
    }
                ...
                <div className='flex flex-col gap-5 mt-6'>
                    <label className='block'>
                        <span className='block text-dark leading-normal -tracking-wide mb-1'>
                            Пользовательское имя. Будет испоьлзоваться для входа в кабинет.
                        ...
                        <Input
                            ...
                            className={clsx(
                                'md:max-w-[485px]',
                                {
                                    'border-rose-500': registerError || state.errors?.username
                                }
                            )}
                            name='username'
                            required
                    ...
                    {registerError &&
                        <div className='text-rose-500 text-[14px]'>
                            Пользователь с таким пользовательским именем или E-mail уже существует!
                        </div>
                    }
                </div>
            </div>
